...
```{r}
duplicated_sequences <- cbind(data.frame(ERCC_exp@elementMetadata$Template), data.frame(ERCC_exp@elementMetadata$miRNA), data.frame(ERCC_exp@assays@data$total_counts))
```


```{r}
colnames(duplicated_sequences)[c(1,2)] = c('sequence', 'mirna')

```

```{r}
duplicated_sequences <- duplicated_sequences[duplicated(duplicated_sequences$sequence),]
#duplicated_sequences = duplicated_sequences %>% group_by(sequence)
```
